...
- `image.repository` - Harbor registry path (harbor.girok.dev/my-girok/web-girok)
...
| `image.repository`        | Image repository                     | `harbor.girok.dev/my-girok/web-girok` |
...
docker build \
  ...
  -t harbor.girok.dev/my-girok/web-girok:v0.1.0 \
  -f apps/web-girok/Dockerfile \
  .

# Push to Harbor registry
docker push harbor.girok.dev/my-girok/web-girok:v0.1.0
```
...
docker build \
  ...
  -t harbor.girok.dev/my-girok/web-girok:develop \
  -f apps/web-girok/Dockerfile .
docker push harbor.girok.dev/my-girok/web-girok:develop

# Main Branch (release)
docker build \
  --build-arg VITE_API_URL=https://my-api.example.com/auth \
  --build-arg VITE_PERSONAL_API_URL=https://my-api.example.com/personal \
  -t harbor.girok.dev/my-girok/web-girok:release \
  -f apps/web-girok/Dockerfile .
docker push harbor.girok.dev/my-girok/web-girok:release

# Production (version tag)
docker build \
  --build-arg VITE_API_URL=https://my-api.example.com/auth \
  --build-arg VITE_PERSONAL_API_URL=https://my-api.example.com/personal \
  -t harbor.girok.dev/my-girok/web-girok:v1.0.0 \
  -f apps/web-girok/Dockerfile .
docker push harbor.girok.dev/my-girok/web-girok:v1.0.0
```
...
```bash
# List releases
helm history web-girok -n my-girok-prod

# Rollback to previous version
helm rollback web-girok -n my-girok-prod

# Rollback to specific revision
helm rollback web-girok 1 -n my-girok-prod
```

## Uninstall

```bash
helm uninstall web-girok --namespace my-girok-prod
```
...
kind: NetworkPolicy
metadata:
  name: web-girok-netpol
spec:
  podSelector:
    matchLabels:
      app.kubernetes.io/name: web-girok
  policyTypes:
    - Ingress
  ingress:
    - from:
...
```bash
kubectl logs -f deployment/web-girok -n my-girok-prod
```
...
```bash
kubectl get pods -n my-girok-prod -l app.kubernetes.io/name=web-girok
```
...
```bash
kubectl get svc -n my-girok-prod -l app.kubernetes.io/name=web-girok
```
...
kubectl describe ingress web-girok -n my-girok-prod
```
...
# Deploy
helm upgrade --install web-girok ./apps/web-girok/helm \
  -f apps/web-girok/helm/values-dev.yaml \
  --namespace my-girok-dev \
  --set image.tag=develop
```
...
# Deploy
helm upgrade --install web-girok ./apps/web-girok/helm \
  -f apps/web-girok/helm/values-prod.yaml \
  --namespace my-girok-prod \
  --set image.tag=release
```
...
docker build \
  --build-arg VITE_API_URL=https://my-api.example.com/auth \
  --build-arg VITE_PERSONAL_API_URL=https://my-api.example.com/personal \
  -t harbor.girok.dev/my-girok/web-girok:v0.2.0 \
  -f apps/web-girok/Dockerfile .
docker push harbor.girok.dev/my-girok/web-girok:v0.2.0
...
```yaml
# .github/workflows/deploy-web-girok-production.yml
      ...
      - name: Build and push Docker image
        run: |
          docker build \
            --build-arg VITE_API_URL=https://my-api.example.com/auth \
            --build-arg VITE_PERSONAL_API_URL=https://my-api.example.com/personal \
            -t harbor.girok.dev/my-girok/web-girok:${{ steps.tag.outputs.tag }} \
            -f apps/web-girok/Dockerfile .
          docker push harbor.girok.dev/my-girok/web-girok:${{ steps.tag.outputs.tag }}
