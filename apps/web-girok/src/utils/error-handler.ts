 ...
 */
function shouldRetryError(statusCode?: number, code?: string): boolean {
...
export function parseAxiosError(error: AxiosError): AppError {
  ...
  const serverMessage =
    (responseData?.message as string) || (responseData?.error as string) || undefined;
  ...
  return {
    ...
    shouldRetry: shouldRetryError(statusCode, error.code),
...
export async function withRetry<T>(
  ...
  while (attempt < retryConfig.maxRetries) {
    try {
      ...
      if (import.meta.env.DEV) {
        console.info(`Retrying after ${delay}ms (attempt ${attempt}/${retryConfig.maxRetries})`);
      }
...
export function getValidationErrors(error: unknown): Record<string, string> | null {
  ...
  if (responseData?.fieldErrors && typeof responseData.fieldErrors === 'object') {
    return responseData.fieldErrors as Record<string, string>;
  }
