# Valkey Caching - 2026 Best Practices

> Caching patterns, ioredis compatibility, session management | **Researched**: 2026-01-22
...
### Performance Improvements (Valkey 8.x)

- **37% higher throughput** on writes
...
```typescript
// Zero code changes - just change import
...
### Snapshot Migration (Fastest)

```bash
# 1. Create snapshot on Redis
redis-cli BGSAVE

# 2. Copy RDB file to Valkey
cp /var/lib/redis/dump.rdb /var/lib/valkey/dump.rdb

# 3. Start Valkey (reads snapshot on startup)
...
```bash
# Use Valkey as replica of Redis, then promote
...
| `volatile-lru` | Mix of cache + persistent |
...
| Don't                    | Do                    | Reason            |
| ------------------------ | --------------------- | ----------------- |
| `KEYS *` in production   | Use `SCAN`            | Blocks server     |
| Large values (>100KB)    | Split or compress     | Memory/latency    |
| No TTL on cache keys     | Always set TTL        | Memory leaks      |
