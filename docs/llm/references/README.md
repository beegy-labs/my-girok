# 2026 Best Practices References

> External knowledge base to reduce web searches | **Last Updated**: 2026-01-22

## Index

| Topic                 | File                          | Key Points                     |
| --------------------- | ----------------------------- | ------------------------------ |
| RAG & Chunking        | `rag-chunking-2026.md`        | 300-800 tokens, semantic-first |
| Frontend (React 19)   | `frontend-react-2026.md`      | Compiler, use(), RSC           |
| Backend (NestJS)      | `backend-nestjs-2026.md`      | Modules, DI, gRPC              |
| Database (PostgreSQL) | `database-postgresql-2026.md` | UUIDv7, VACUUM, pgvector       |
| Testing               | `testing-2026.md`             | Pyramid, 80% coverage          |
| Security              | `security-2026.md`            | OWASP Top 10, DevSecOps        |
| CI/CD                 | `cicd-devops-2026.md`         | GitHub Actions, GitOps         |
| API Design            | `api-design-2026.md`          | REST, GraphQL, gRPC            |
| Kubernetes            | `kubernetes-2026.md`          | HPA, security, Helm            |

## Usage

These documents are **reference material** from 2026 industry best practices.
Consult when making architectural decisions or validating current patterns.

## Update Policy

- **Review**: Quarterly
- **Source**: Web search aggregation
- **Format**: Concise, actionable, with source links

---

_Main: `best-practices.md`_
