# TypeScript - 2026 Best Practices

> Type safety, strict mode, runtime validation | **Researched**: 2026-01-22
...
| `strictPropertyInitialization` | Class properties must initialize |
...
```typescript
// ✅ TypeScript 5+ catch variable handling
try {
  await fetchData();
} catch (error: unknown) {
...
| Don't                  | Do                      | Reason            |
| ---------------------- | ----------------------- | ----------------- |
| `any` everywhere       | `unknown` + type guards | Type safety       |
| Skip `strict` mode     | Enable all strict flags | Catch bugs early  |
| Trust API responses    | Validate with Zod       | Runtime safety    |
| `as` type assertions   | Type guards             | Actual checking   |
| Non-null assertion `!` | Proper null checks      | Explicit handling |
