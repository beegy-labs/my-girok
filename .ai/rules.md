...
```yaml
LLM_DOCUMENTATION_RULE:
  ONLY_EDIT:
    - .ai/* # Indicator files
    - docs/llm/* # SSOT (Source of Truth)
  NEVER_EDIT:
    - docs/en/* # Generated by admin action
    - docs/kr/* # Translated by admin action
  GENERATION:
    trigger: Manual (admin button/command only)
    commands:
      - pnpm docs:generate # llm/ → en/
      - pnpm docs:translate # en/ → kr/
    NOT: Auto-sync or CI/CD pipeline
```
