...
| 요소             | 기준 | 예시                                                   |
| ---------------- | ---- | ------------------------------------------------------ |
| 본문 텍스트      | 7:1  | `text-theme-text-primary`, `text-theme-text-secondary` |
| 버튼 텍스트      | 7:1  | Button 내부 텍스트                                     |
| 링크 텍스트      | 7:1  | `text-theme-primary`                                   |
| 레이블           | 7:1  | Form label, 카드 제목                                  |
| 에러/상태 메시지 | 7:1  | Alert, validation messages                             |

### WCAG AA 허용 (목업 우선)

| 요소             | AA 기준 | 목업 값                       | 비고                           |
| ---------------- | ------- | ----------------------------- | ------------------------------ |
| **터치 타겟**    | 24x24px | 40x40px                       | 목업 40px 사용 (AAA 44px 미달) |
| **보더**         | 3:1     | `border-theme-border-default` | UI 컴포넌트 대비 (SC 1.4.11)   |
| **아이콘**       | 3:1     | `text-theme-text-secondary`   | 의미 전달용 아이콘             |
| **플레이스홀더** | 4.5:1   | `text-theme-text-muted`       | 입력 힌트 텍스트               |
| **비활성 요소**  | N/A     | `opacity-50`                  | 상호작용 불가 상태             |
| **장식 요소**    | N/A     | 그림자, 배경 그라데이션       | 정보 전달 목적 아님            |
| **포커스 링**    | 3:1     | `ring-theme-focus-ring`       | 포커스 표시자                  |
| **큰 텍스트**    | 4.5:1   | 18pt+ 또는 14pt bold          | 제목, 헤딩 (선택적 완화)       |

### 목업 우선 레이아웃

| 요소            | 목업 스펙          | WCAG 권장 | 정책          |
| --------------- | ------------------ | --------- | ------------- |
| 핀 버튼 (Grid)  | `w-10 h-10` (40px) | 44px      | **목업 우선** |
| 핀 버튼 (List)  | `p-2` (~36px)      | 44px      | **목업 우선** |
| ViewToggle 버튼 | `p-2` (~34px)      | 44px      | **목업 우선** |
| 카드 패딩       | `p-10` (40px)      | -         | **목업 우선** |
| 카드 라운딩     | `rounded-[40px]`   | -         | **목업 우선** |
| 보더 두께       | `border-2`         | -         | **목업 우선** |

### Rationale

- **AAA 필수 (텍스트)**: 사회적 약자 배려, 가독성이 핵심 기능
- **AA 허용 (UI)**: 터치 타겟 40px는 AA (24px) 초과, 시각적 일관성 우선
- **목업 우선**: V24.5 디자인 시스템의 일관성이 4px 차이보다 중요
