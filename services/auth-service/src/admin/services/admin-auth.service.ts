  ...
  async getProfile(adminId: string): Promise<AdminProfileResponse> {
    ...
    return {
      ...
      tenant: admin.tenantId
        ? {
            id: admin.tenantId,
            name: (admin as Record<string, unknown>).tname as string,
            slug: admin.tenantSlug!,
            type: admin.tenantType!,
            status: (admin as Record<string, unknown>).tenantStatus as string,
          }
        : undefined,
      role: {
        id: admin.roleId,
        name: admin.roleName,
        displayName: (admin as Record<string, unknown>).roleDisplayName as string,
        level: (admin as Record<string, unknown>).roleLevel as number,
      },
      permissions,
