 ...
 * - gRPC microservice for inter-service communication (port 50054)
 * - HTTP health check endpoint
 */

const GRPC_PORT = 50054;
...
async function bootstrap() {
  ...
  app.connectMicroservice<MicroserviceOptions>({
    ...
    options: {
      package: 'mail.v1',
      protoPath: join(__dirname, '../../../packages/proto/mail/v1/mail.proto'),
  ...
  logger.log(`Mail Service gRPC running on port ${grpcPort}`);

  // Start HTTP server (for health checks)
  await app.listen(httpPort);
  logger.log(`Mail Service HTTP running on port ${httpPort}`);
